...
**val.py指令:**
python val.py   -net sam   -mod sam_adpt   -dataset kits   -sam_ckpt ./checkpoint/sam/sam_vit_b_01ec64.pth   -weights ./logs/kits23_Med-SA_train_EPOCH_500_2026_03_16_22_54_00/Model/best_dice_checkpoint.pth   -exp_name test_kits23_Med-SA_train_EPOCH_500_2026_03_16_22_54_00   -vis 1   -gpu_device 0   -image_size 1024   -data_path ../dataset/KiTS23_for_MSA
...
### 2) validation_sam() 的修改 (function.py)
...
2.model中efficient_sam_decoder.py裡將 view(batch_size * max_num_queries, ...) 改為 view(-1, ...)，是為了簡化程式碼，讓形狀的第一個維度自動推斷，減少對顯式變數（如 batch_size 和 max_num_queries）的依賴。這樣更通用，對於不同批次大小或 query 數量時也更具彈性。

3.訓練次數從100次降到2次(以程式碼能夠運行為主) 

4.將批量大小從1024下調到512

5.在執行train.py時，程式碼有執行這段if ind % args.vis == 0: ，但因為args.vis 是 None，所以直接在執行 train.py 時補上 -vis 1 ，這樣 args.vis 就會是 1（整數），不會是 None
用途:「每隔固定次數執行一次某個動作」，是訓練過程中常見的寫法。 

- 	ind 是某個迴圈中的「當前索引值」或「影像編號」
- 	args.vis 是一個參數，通常控制 「每幾張圖要視覺化（或儲存、印出）一次」
- 	% 是 取餘數運算，用來做「每隔 N 次」的條件判斷

6.將Anaconda Prompt的版本從3.13調降至3.10

## 11/30 最新的進度(3D腹部 BTCV)
